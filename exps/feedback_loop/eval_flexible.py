...
def _init_worker(phi_ckpt: str) -> None:
    ...
    phi_fn, _count_fn, _stability_fn = load_inference_functions(
        phi_ckpt=phi_ckpt,
...
def _run_one(task: tuple) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if _WORKER_PHI_FN is None:
        raise RuntimeError("Worker inference function is not initialized")
    phi_u = _WORKER_PHI_FN(theta)
    pred_solutions, _init_centers, _history, _layers = adaptive_peak_detection(
        phi_u,
        D,
        **locator_kwargs,
    )
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="Evaluate flexible clustering on the feedback-loop observation test set.")
    ...
    if num_procs <= 1:
        def predict_fn(theta):
            phi_u = phi_fn(theta)
            centers, _init_centers, _history, _layers = adaptive_peak_detection(
                phi_u,
                D,
                **locator_kwargs,
            )
        ...
        try:
            ...
            with ctx.Pool(
                ...
                for item in tqdm.tqdm(
                    ...
                    results.append(item)
            metrics, details = aggregate_evaluation_results(results, count_source="flexible_final_solution_count")
