...
def _run_one(task: tuple) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    theta, true_solutions, true_stability, D, locator_kwargs = task
...
def main() -> None:
    ...
    p.add_argument("--obs-missing-path", type=str, default=str(resolve_path(exp_dir, cfg_get(data_outputs_cfg, "obs_missing_pkl", "feedback_loop_obs_missing.pkl"))))
